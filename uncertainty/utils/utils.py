...
from myllava.constants import DEFAULT_IMAGE_TOKEN
...
def get_parser(stages=['generate', 'compute']):
    ...
    if 'generate' in stages:
        parser.add_argument(
            "--model_name", type=str, default="Qwen2.5", help="Model name",
        )
        parser.add_argument(
            "--image_path", type=str, default="/data/huangtao/projects/large_models/MMVP/ramdon_no_correct_label", help="",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--dataset", type=str, default="hallucination",
            # choices=['trivia_qa', 'squad', 'bioasq', 'nq', 'svamp', "mmvp", "mllm"],
            help="Dataset to use")
        parser.add_argument(
        ...
        parser.add_argument(
            "--num_samples", type=int, default=10,  # default=400,
            help="Number of samples to use")
        parser.add_argument(
            "--num_few_shot", type=int, default=5,
            help="Number of few shot examples to use")
        parser.add_argument(
            "--p_true_num_fewshot", type=int, default=5,
            help="Number of few shot examples to use")
        parser.add_argument(
        ...
        parser.add_argument('--attention_evidence_metric', default=True,
                            action='store_true')
        parser.add_argument('--llm_head_evidence_metric', default=True,
                            action='store_true')
        parser.add_argument(
            "--brief_always", default=False, action='store_true')
        parser.add_argument(
            "--enable_brief", default=True, action='store_true')
        parser.add_argument(
            "--brief_prompt", default='default', type=str)
        parser.add_argument(
            "--prompt_type", default='mmvp', type=str)
    ...
    if 'compute' in stages:
        ...
        parser.add_argument('--compute_probalibility_method', default=True,
        ...
        parser.add_argument('--strict_entailment',
                            default=True, action='store_true')
        parser.add_argument('--use_all_generations', default=True, action='store_true')
...
def init_model(args):
    ...
    if mn == "Qwen2.5":
        ...
        # model_path = "/data/public/models/Qwen"
        # model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
        #     "Qwen/Qwen2.5-VL-7B-Instruct", torch_dtype="auto", device_map="auto", cache_dir=model_path   
        # )
        ...
        processor = AutoProcessor.from_pretrained("Qwen/Qwen2.5-VL-7B-Instruct")
        models = (None, model, processor, None)
        model = models
    else:
...
def get_metric(metric):
    if metric == 'squad':
        ...
        def metric(response, example, *args, **kwargs):
            ...
            if example['answers']['text'] and example['answers']['text'][0]:
                if 'id' in example:
                    exid = example['id']
                ...
                return 1.0 if (results['f1'] >= 50.0 or response[0].lower()==example['answers']['text'][0][0].lower()) else 0.0
            else:
                return -1
    elif metric == 'llm_1':
        metric = llm_metric
...
def save_csv(df, file):
    full_path = os.path.join(wandb.run.dir, file)
    df.to_csv(full_path)
