...
from uncertainty.utils.eval_utils import (
    ...
    area_under_thresholded_accuracy,aupr)
import pandas as pd
...
def init_wandb(wandb_runid, assign_new_wandb_id, experiment_lot, entity):
    ...
    if not assign_new_wandb_id:
        ...
        old_run = api.run(f'{entity}/EUQ/{wandb_runid}')
...
def uncertainty_dict_to_csv(data_dict):
    ...
    metrics = list(data_dict["uncertainty"][methods[0]].keys())  
...
def analyze_run(
    ...
    eval_metrics = dict(zip(['AUROC', "AUPR",'area_under_thresholded_accuracy', 'mean_uncertainty'],[auroc, aupr,area_under_thresholded_accuracy, np.mean]))
    ...
    for measure_name, measure_values in rum.items():
        ...
        for validation_is_false, logging_name in zip(validation_is_falses, logging_names):
            ...
            fargs = {
                ...
                'AUPR': [validation_is_false, measure_values],
